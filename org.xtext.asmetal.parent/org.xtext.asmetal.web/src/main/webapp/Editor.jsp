	...
	<div class="run">
		...
		<script language="JavaScript">
			function esegui() {
				...
				var cut = tes.substring(50, tes.length);
				var counter = 0;
				var testo = "";
				var fine = 0;
				var aum = 0;
				...
				while (!(cut.substring(fine, fine + 52) == chiudi)) {
					if (cut.substring(fine, fine + 48) == acapo) {
						testo = testo
								.concat(cut.substring(counter, fine), "\n");
						counter = fine + 48;
						fine = fine + 48;
					} else {
						if (cut.substring(fine, fine + 26) == pChiave)
							aum = 26;
						else if (cut.substring(fine, fine + 7) == spChiave)
							aum = 7
						else if (cut.substring(fine, fine + 39) == num)
							aum = 39;
						else if (cut.substring(fine, fine + 25) == str)
							aum = 25;
						else if (cut.substring(fine, fine + 31) == tab)
							aum = 31;
						else
							fine++;
				...
				while (counter < testo.length) {
					while ((testo.charAt(counter)) != "&"
							&& (counter < testo.length)) {
						counter++;
					}
					if ((testo.substring(counter, counter + 4)) == mag
							|| (testo.substring(counter, counter + 4) == min)) {
						tes = tes.concat(testo.substring(fine, counter));
						if (testo.charAt(counter + 1) == "g")
							tes = tes.concat(">");
						else
							tes = tes.concat("<");
						fine = counter + 4;
